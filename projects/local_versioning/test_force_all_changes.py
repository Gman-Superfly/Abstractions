...
# Track GC stats but don't spam output
# gc.set_debug(gc.DEBUG_STATS)  # Disabled - too spammy
...
@CallableRegistry.register("modify_all_entities")
def modify_all_entities(gridmap: GridMap) -> GridMap:
    """
    Modify ALL entities in the gridmap.
    
    This modifies every entity (gridmap, all nodes, all agents) to force
    the diff algorithm to detect 111 changes with force_versioning=False.
    
    Args:
        gridmap: The grid map entity
    
    Returns:
        Mutated gridmap with all entities modified
    """
    # Modify gridmap itself
    gridmap.untyped_data = f"modified_{time.time()}"
    
    # Modify ALL nodes
    for node in gridmap.nodes:
        node.untyped_data = f"modified_{time.time()}"
        
        # Modify ALL agents
        for agent in node.agents:
            agent.untyped_data = f"modified_{time.time()}"
...
def stress_test_moves(
    ...
    metrics = PerformanceMetrics(
        operation_name=f"modify_all_entities_{num_nodes}x{agents_per_node}",
        num_nodes=num_nodes,
    ...
    # Perform operations - modify ALL entities each time
    print(f"Performing {num_operations} full-modification operations (ALL entities changed)...")
    
    # Track GC time with callbacks
    gc_time_tracker = {'total': 0.0}
    
    def gc_callback(phase, info):
        """Track GC collection time"""
        if phase == 'start':
            gc_time_tracker['start'] = time.perf_counter()
        elif phase == 'stop':
            if 'start' in gc_time_tracker:
                gc_time_tracker['total'] += (time.perf_counter() - gc_time_tracker['start']) * 1000
    
    # Register GC callback
    gc.callbacks.append(gc_callback)
    
    # Track profiling stats every 10 operations
    window_stats = []
    
    # Track operation timing separately
    total_execute_time = []
    
    for i in range(num_operations):
        # Capture profiling state before operation
        if i % 10 == 0:
            snapshot_before = {
                'build': EntityRegistry._total_build_time,
                'diff': EntityRegistry._total_diff_time,
                'update_ids': EntityRegistry._total_update_ids_time,
                'update_map': EntityRegistry._total_update_mappings_time,
                'register': EntityRegistry._total_register_time,
                'version_count': EntityRegistry._version_count,
                'gc_time': gc_time_tracker['total'],
                # CallableRegistry overhead tracking
                'input_copy': CallableRegistry._total_input_copy_time,
                'function_exec': CallableRegistry._total_function_exec_time,
                'semantic': CallableRegistry._total_semantic_time,
            }
        
        # Time the full execute call (includes input validation, function call, versioning)
        start = time.perf_counter()
        ...
        total_execute_time.append(duration_ms)
        
        # Capture profiling deltas every 10 operations
        if (i + 1) % 10 == 0:
            registry_size = len(EntityRegistry.tree_registry)
            recent_avg = sum(metrics.timings[-10:]) / 10
            
            # Calculate deltas for this window
            build_delta = EntityRegistry._total_build_time - snapshot_before['build']
            diff_delta = EntityRegistry._total_diff_time - snapshot_before['diff']
            update_ids_delta = EntityRegistry._total_update_ids_time - snapshot_before['update_ids']
            update_map_delta = EntityRegistry._total_update_mappings_time - snapshot_before['update_map']
            register_delta = EntityRegistry._total_register_time - snapshot_before['register']
            gc_delta = gc_time_tracker['total'] - snapshot_before['gc_time']
            
            # CallableRegistry overhead deltas
            input_copy_delta = CallableRegistry._total_input_copy_time - snapshot_before['input_copy']
            function_exec_delta = CallableRegistry._total_function_exec_time - snapshot_before['function_exec']
            semantic_delta = CallableRegistry._total_semantic_time - snapshot_before['semantic']
            
            # Calculate time spent
            version_entity_time = build_delta + diff_delta + update_ids_delta + update_map_delta + register_delta  # Time inside version_entity
            callable_overhead = input_copy_delta + function_exec_delta + semantic_delta  # Time in CallableRegistry
            actual_time = sum(metrics.timings[-10:])  # Total wall time for 10 operations
            unaccounted = actual_time - version_entity_time - callable_overhead - gc_delta  # Remaining overhead
            ...
            window_stats.append({
                ...
                'build': build_delta,
                'diff': diff_delta,
                'update_ids': update_ids_delta,
                'update_map': update_map_delta,
                'register': register_delta,
                'version_entity': version_entity_time,
                'input_copy': input_copy_delta,
                'function_exec': function_exec_delta,
                'semantic': semantic_delta,
                'callable_overhead': callable_overhead,
                'gc_time': gc_delta,
                'actual': actual_time,
                'unaccounted': unaccounted,
                'total_avg': recent_avg,
            })
            
            print(f"  Progress: {i + 1}/{num_operations} operations "
                  f"(avg: {recent_avg:.3f} ms/op, registry_size: {registry_size})")
    ...
    EntityRegistry.print_profiling_stats()
    
    # Print window-by-window breakdown to see growth
    print("\n" + "="*130)
    print("TIMING BREAKDOWN BY WINDOW (10 operations each)")
    print("="*130)
    print(f"{'Ops':<10} {'Reg':<6} {'VER_ENT':<12} {'INP_COPY':<12} {'FUNC':<10} {'SEMANTIC':<12} {'GC':<12} {'UNACC':<12} {'ACTUAL':<10}")
    print("-"*130)
    for w in window_stats:
        ver_pct = (w['version_entity'] / w['actual'] * 100) if w['actual'] > 0 else 0
        copy_pct = (w['input_copy'] / w['actual'] * 100) if w['actual'] > 0 else 0
        sem_pct = (w['semantic'] / w['actual'] * 100) if w['actual'] > 0 else 0
        gc_pct = (w['gc_time'] / w['actual'] * 100) if w['actual'] > 0 else 0
        unacc_pct = (w['unaccounted'] / w['actual'] * 100) if w['actual'] > 0 else 0
        print(f"{w['ops']:<10} {w['registry_size']:<6} "
              f"{w['version_entity']:>6.1f}ms {ver_pct:>3.0f}%  "
              f"{w['input_copy']:>6.1f}ms {copy_pct:>3.0f}%  "
              f"{w['function_exec']:>5.1f}ms  "
              f"{w['semantic']:>6.1f}ms {sem_pct:>3.0f}%  "
              f"{w['gc_time']:>6.1f}ms {gc_pct:>3.0f}%  "
              f"{w['unaccounted']:>6.1f}ms {unacc_pct:>3.0f}%  "
              f"{w['actual']:>8.1f}ms")
    
    # Print key findings
    print("="*130)
    print("\n🔍 KEY FINDINGS:")
    print(f"   INP_COPY: {window_stats[0]['input_copy']:.1f}ms → {window_stats[-1]['input_copy']:.1f}ms (Pydantic model_copy GROWS!)")
    print(f"   SEMANTIC: {window_stats[0]['semantic']:.1f}ms → {window_stats[-1]['semantic']:.1f}ms (includes version_entity)")
    print(f"   GC:       {window_stats[0]['gc_time']:.1f}ms → {window_stats[-1]['gc_time']:.1f}ms (garbage collection)")
    print(f"   VER_ENT:  {window_stats[0]['version_entity']:.1f}ms → {window_stats[-1]['version_entity']:.1f}ms (CONSTANT ✓)")
    print("="*130 + "\n")
